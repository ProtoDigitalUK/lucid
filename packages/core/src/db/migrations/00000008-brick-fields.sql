...
CREATE TABLE IF NOT EXISTS lucid_groups (
  ...
  language_code TEXT REFERENCES lucid_languages(code) ON DELETE CASCADE,
  repeater_key TEXT NOT NULL,
  group_order INT NOT NULL,
  ref TEXT
);
CREATE INDEX idx_lucid_groups_language_code ON lucid_groups(language_code);
...
CREATE INDEX idx_lucid_fields_language_code ON lucid_fields(language_code);
