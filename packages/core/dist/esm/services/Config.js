...
export default class Config {
    ...
    static findPath = (cwd) => {
        ...
        search(cwd);
        if (!configPath) {
            throw new RuntimeError("Cannot find the lucid.config.ts or lucid.config.js file at the root of your project.");
        }
        return configPath;
    };
    ...
    static get media() {
        ...
        return {
            ...
            processedImageLimit: media?.processedImageLimit || C.media.processedImageLimit,
            store: {
    ...
    static #validateBricks(config) {
        ...
        if (brickKeys.length !== uniqueBrickKeys.length) {
            throw new RuntimeError("Each brick key must be unique, found duplicates in lucid.config.ts/js.");
        }
    }
    static #validateCollections(config) {
        ...
        if (collectionKeys.length !== uniqueCollectionKeys.length) {
            throw new RuntimeError("Each collection key must be unique, found duplicates in lucid.config.ts/js.");
        }
    }
    static #validateForms(config) {
        ...
        if (formKeys.length !== uniqueFormKeys.length) {
            throw new RuntimeError("Each form key must be unique, found duplicates in lucid.config.ts/js.");
        }
    }
}
